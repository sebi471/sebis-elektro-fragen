...
let correct=0,wrong=0,open=0, currentTopic='';
...
function buildTopics(){
  ...
  correct=wrong=open=0; updateStats();
  Object.keys(questions).forEach(t=>{
    const card=document.createElement('div');
    card.className='topic-card';
    card.innerHTML=`<img src="img/${t.toLowerCase()}.png"><p>${t}</p>`;
...
function startQuiz(topic){
  ...
  const data=progress[topic]||{correct:0,wrong:0,index:0};
  let idx=data.index, correctCount=data.correct, wrongCount=data.wrong;
  const list=[...questions[topic]];
  correct=correctCount;wrong=wrongCount;open=list.length-idx;updateStats();show();
  function show(){
    if(idx>=list.length){
